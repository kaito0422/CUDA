...
/* 由于在kernel中必须知道图片长宽，此处添加_width 和 _height */
__global__ void kernelImageProcessing(cuda_int *image, cuda_int *out, cuda_int *_width, cuda_int *_height)
{
  ...
  /* 由于thread空间的width和height可能会超过图片的width和height，多余的thread就不参与计算图片像素 */
	if ((idx < width && idy < height)) {
	//	unsigned int thread_idx = blockDim.x * blockDim.y * (gridDim.x * blockIdx.y + blockIdx.x) + \
	//		blockDim.x * threadIdx.y + threadIdx.x;
  /* 因此对于当前thread的输入数据的计算必须用idx和idy来计算得到 */
		unsigned int thread_idx = idy * width + idx;
	...
	//	if (dif1 < 0x12 && dif2 < 0x12 && dif3 < 0x12)
	//		data = 0xFF000000;
	//	data = 0xFF000000 | red << 16 | green << 0 | blue << 8;
		red = (unsigned char)(((double)(data & 0xFFFFFF) / (double)(0xFFFFFF)) * 0xFF);
		green = (unsigned char)(((double)(data & 0xFFFFFF) / (double)(0xFFFFFF)) * 0xFF);
		blue = (unsigned char)(((double)(data & 0xFFFFFF) / (double)(0xFFFFFF)) * 0xFF);
		data = 0xFF000000 | red << 16 | green << 8 | blue;
		out[thread_idx] = data;
	}
}
...
void processImage(const char *filename)
{
	...
	cudaMalloc((void **)&dev_out, width * height * sizeof(cuda_int));
	cudaMalloc((void **)&dev_width, sizeof(cuda_int));
	cudaMalloc((void **)&dev_height, sizeof(cuda_int));

	cudaMemcpy(dev_image, imageBuf, sizeof(cpu_int) * width * height, cudaMemcpyHostToDevice);
	cudaMemcpy(dev_width, &width, sizeof(cpu_int), cudaMemcpyHostToDevice);
	cudaMemcpy(dev_height, &height, sizeof(cpu_int), cudaMemcpyHostToDevice);

  /* 此处为自适应block维度，其中要注意一个block内的thread数量不能查过1024（MX250的极限） */
	int block_x, block_y, thread_x, thread_y;
	block_x = width % 32 ? width / 32 + 1 : width / 32;
	block_y = height % 32 ? height / 32 + 1 : height / 32;
	dim3 threads_rect(32, 32);				// block的维度，每个block有32x32个thread
	dim3 blocks_rect(block_x, block_y);		// grid的维度

	kernelImageProcessing<<<blocks_rect, threads_rect>>>(dev_image, dev_out, dev_width, dev_height);

	cudaMemcpy(outBuf, dev_out, sizeof(cpu_int) * width * height, cudaMemcpyDeviceToHost);

	std::string outname(filename);
	auto iter = std::find(outname.begin(), outname.end(), '.');
	outname.erase(iter, outname.end());
	outname.append("_new.png");
	saveImage(outname.data(), outBuf, width, height);
}
...
//	test1DKernel();
//	test2DKernel();
	processImage("abc.jpg");
	processImage("atheris.jpeg");
	processImage("flash.jpg");
	processImage("hyperbeast01.png");
	processImage("hyperbeast02.png");
	processImage("hyperbeast03.jpg");
	processImage("hyperbeast04.jpg");
	processImage("zoom.jpg");
